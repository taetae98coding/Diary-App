...
import io.github.taetae98coding.diary.core.database.transaction.account.AccountMemoTransaction
...
internal class AccountMemoRepositoryImpl(
    ...
    override suspend fun add(account: Account, memo: Memo, memoTagIds: Set<Uuid>) {
        transactor.transaction {
            accountMemoTransaction.upsert(account.id, listOf(memo.toLocal()))
            memoTagLocalDataSource.upsert(memoTagIds.map { MemoTagLocalEntity(memo.id, it, true, clock.now().toEpochMilliseconds()) })
            when (account) {
