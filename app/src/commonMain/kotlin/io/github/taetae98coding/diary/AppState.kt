...
internal class AppState(
    ...
    val isNavigationVisible by derivedStateOf {
        ...
        val isVisibleNavKey = backStack.lastOrNull() in listOf(ListMemoFilterNavKey, CalendarMemoFilterNavKey)
    ...
    fun navigate(destination: TopLevelDestination) {
        val topLevelNavKeyList = topLevelDestination.map(TopLevelDestination::navKey)
        while (backStack.lastOrNull() !in topLevelNavKeyList) {
            backStack.removeLastOrNull()
        }

        if (backStack.lastOrNull() == destination.navKey) return

        backStack.removeAll { it in topLevelNavKeyList && it != CalendarNavKey }
...
internal fun rememberAppState(): AppState {
    val backStack = rememberNavBackStack(CalendarNavKey)
