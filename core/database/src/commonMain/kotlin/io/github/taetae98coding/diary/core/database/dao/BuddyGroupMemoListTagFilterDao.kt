...
@Dao
internal abstract class BuddyGroupMemoListTagFilterDao : RoomDao<MemoListTagFilterLocalEntity>() {
    @Query(
        """
                SELECT COUNT(*) != 0
                ...
                WHERE MemoListTagFilter.isFilter
        """,
    )
    ...
    @Query(
        ...
        SELECT
            Tag.*,
            COALESCE(MemoListTagFilter.isFilter, 0) AS isFilter
        FROM Tag
        INNER JOIN BuddyGroupTag ON BuddyGroupTag.tagId = Tag.id AND BuddyGroupTag.buddyGroupId = :buddyGroupId
        LEFT JOIN MemoListTagFilter ON MemoListTagFilter.tagId = Tag.id
