...
@Factory
public class MemoRemoteDataSource internal constructor(
    @param:DiaryClient
    private val httpClient: HttpClient,
) {
    public suspend fun get(token: String, memoId: Uuid): DiaryRemoteEntity<MemoRemoteEntity> {
        val response = httpClient.get("/v1/memo/$memoId") {
    ...
    public suspend fun upsert(
        token: String,
        entity: List<MemoRemoteEntity>,
    ): DiaryRemoteEntity<Unit> {
        val response = httpClient.post("/v1/memo") {
            header(HttpHeaders.Authorization, "Bearer $token")
