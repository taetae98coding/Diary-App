...
@Factory
public class AccountTagRemoteDataSource internal constructor(
    @param:DiaryClient
    private val httpClient: HttpClient,
) {
    public suspend fun fetch(token: String, lastUpdatedAt: Long): DiaryRemoteEntity<List<TagRemoteEntity>> {
        val response = httpClient.get("/v1/tag") {
            header(HttpHeaders.Authorization, "Bearer $token")
            parameter("lastUpdatedAt", lastUpdatedAt)
        }
