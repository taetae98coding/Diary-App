        ...
        Given("세션이 null로 조회되는 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Account.Guest 를 방출한다") {
                    result.getOrThrow().shouldBeInstanceOf<Account.Guest>()
                }
            }
        }

        Given("세션이 존재하는 경우") {
            val session = fixture.giveMeKotlinBuilder<Session>()
                .sample()
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Account.User 로 매핑되어 방출한다") {
                    val account = result.getOrThrow().shouldBeInstanceOf<Account.User>()
