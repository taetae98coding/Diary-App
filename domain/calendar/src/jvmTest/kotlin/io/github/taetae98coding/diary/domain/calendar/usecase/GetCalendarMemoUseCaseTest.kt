...
class GetCalendarMemoUseCaseTest : BehaviorSpec() {
    init {
        ...
        Given("사용자의 기간 메모를 조회하는 경우") {
            ...
            val memos = fixture.giveMe<CalendarMemo>(10)
            every { getAccountUseCase() } returns flowOf(Result.success(account))
            every { calendarMemoRepository.get(account, dateRange) } returns flowOf(memos)
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 결과를 그대로 방출한다") {
                    result.getOrThrow().shouldContainExactly(memos)
                }
