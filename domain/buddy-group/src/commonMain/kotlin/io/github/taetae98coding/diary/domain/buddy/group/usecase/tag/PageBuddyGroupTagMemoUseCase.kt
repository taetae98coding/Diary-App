...
@Factory
public class PageBuddyGroupTagMemoUseCase internal constructor(
    private val getAccountUseCase: GetAccountUseCase,
    private val buddyGroupTagMemoRepository: BuddyGroupTagMemoRepository,
) {
    public operator fun invoke(buddyGroupId: Uuid, tagId: Uuid): Flow<Result<PagingData<Memo>>> {
        return flow {
            getAccountUseCase().mapLatest { it.getOrThrow() }
                .flatMapLatest { account ->
                    when (account) {
                        is Account.Guest -> flowOf(Result.failure(NotLoginException()))
                        is Account.User -> buddyGroupTagMemoRepository.page(account, buddyGroupId, tagId)
