...
@Factory
public class PageBuddyGroupBuddyUseCase internal constructor(
    private val getAccountUseCase: GetAccountUseCase,
    private val buddyGroupBuddyRepository: BuddyGroupBuddyRepository,
) {
    public operator fun invoke(buddyGroupId: Uuid): Flow<Result<PagingData<Buddy>>> {
        return flow {
            getAccountUseCase().mapLatest { it.getOrThrow() }
                .flatMapLatest { account ->
                    when (account) {
                        is Account.Guest -> flowOf(Result.failure(NotLoginException()))
                        is Account.User -> buddyGroupBuddyRepository.page(account, buddyGroupId).mapLatest { Result.success(it) }
                    }
                }
                .also { emitAll(it) }
        }.catch {
