...
@Factory
public class RestartBuddyGroupMemoUseCase internal constructor(
    ...
    public suspend operator fun invoke(buddyGroupId: Uuid, memoId: Uuid): Result<Unit> {
        return runCatching {
            when (val account = getAccountUseCase().first().getOrThrow()) {
                ...
                is Account.User -> buddyGroupMemoRepository.updateFinished(account, buddyGroupId, memoId, false)
            }
        }
    }
}
