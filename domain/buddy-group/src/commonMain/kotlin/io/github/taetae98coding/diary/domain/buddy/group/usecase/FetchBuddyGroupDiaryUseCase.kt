...
@Factory
public class FetchBuddyGroupDiaryUseCase internal constructor(
    private val getAccountUseCase: GetAccountUseCase,
    private val buddyGroupDiaryRepository: BuddyGroupDiaryRepository,
) {
    public suspend operator fun invoke(buddyGroupId: Uuid): Result<Unit> {
        return runCatching {
            when (val account = getAccountUseCase().first().getOrThrow()) {
                is Account.Guest -> throw NotLoginException()
                is Account.User -> buddyGroupDiaryRepository.fetch(account, buddyGroupId)
            }
        }
    }
}
