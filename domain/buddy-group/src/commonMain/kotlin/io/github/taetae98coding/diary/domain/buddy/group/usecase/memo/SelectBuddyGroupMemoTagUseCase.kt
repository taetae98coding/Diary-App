...
@Factory
public class SelectBuddyGroupMemoTagUseCase internal constructor(
    ...
    public suspend operator fun invoke(buddyGroupId: Uuid, memoId: Uuid, tagId: Uuid): Result<Unit> {
        return runCatching {
            when (val account = getAccountUseCase().first().getOrThrow()) {
                ...
                is Account.User -> buddyGroupMemoTagRepository.updateMemoTag(account, buddyGroupId, memoId, tagId, true)
            }
        }
    }
}
