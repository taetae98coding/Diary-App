...
class PageBuddyGroupSelectMemoTagUseCaseTest : BehaviorSpec() {
    init {
        ...
        val buddyGroupSelectMemoTagRepository = mockk<BuddyGroupSelectMemoTagRepository>(relaxed = true, relaxUnitFun = true)
        val useCase = PageBuddyGroupSelectMemoTagUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupSelectMemoTagRepository = buddyGroupSelectMemoTagRepository,
        )

        Given("로그인된 계정으로 그룹의 메모 태그를 페이징하는 경우") {
            val account = fixture.giveMeOne<Account.User>()
            val buddyGroupId = fixture.giveMeOne<Uuid>()
            val memoId = fixture.giveMeOne<Uuid>()
            val pagingData = PagingData.empty<SelectMemoTag>()

            every { getAccountUseCase() } returns flowOf(Result.success(account))
            every { buddyGroupSelectMemoTagRepository.page(account, buddyGroupId, memoId) } returns flowOf(pagingData)

            When("유즈케이스를 구독하면") {
                val result = useCase(buddyGroupId, memoId).first()
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupSelectMemoTagRepository.page(account, buddyGroupId, memoId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupSelectMemoTagRepository.page(any(), any(), any())
                    }
                }
            }
        }
