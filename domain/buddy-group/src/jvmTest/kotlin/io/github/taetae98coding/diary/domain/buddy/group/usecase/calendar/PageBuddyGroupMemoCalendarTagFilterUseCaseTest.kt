...
class PageBuddyGroupMemoCalendarTagFilterUseCaseTest : BehaviorSpec() {
    init {
        ...
        val useCase = PageBuddyGroupMemoCalendarTagFilterUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupMemoCalendarTagFilterRepository = buddyGroupMemoCalendarTagFilterRepository,
        )

        Given("로그인된 계정으로 그룹의 캘린더 메모 필터를 페이징으로 조회하는 경우") {
            ...
            every { buddyGroupMemoCalendarTagFilterRepository.page(buddyGroupId) } returns flowOf(paging)
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupMemoCalendarTagFilterRepository.page(buddyGroupId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupMemoCalendarTagFilterRepository.page(any())
                    }
                }
            }
        }
