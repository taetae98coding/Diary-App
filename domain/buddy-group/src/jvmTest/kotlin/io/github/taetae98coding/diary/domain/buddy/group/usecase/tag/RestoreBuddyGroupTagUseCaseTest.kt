...
class RestoreBuddyGroupTagUseCaseTest : BehaviorSpec() {
    init {
        ...
        val useCase = RestoreBuddyGroupTagUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupTagRepository = buddyGroupTagRepository,
        )

        Given("로그인된 계정으로 태그를 복원하는 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository를 호출한다") {
                    coVerify(exactly = 1) {
                        buddyGroupTagRepository.updateDeleted(
                            account = account,
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository를 호출하지 않는다") {
                    coVerify(exactly = 0) {
                        buddyGroupTagRepository.updateDeleted(any(), any(), any(), any())
                    }
                }
            }
        }
