...
class PageBuddyGroupMemoListTagFilterUseCaseTest : BehaviorSpec() {
    init {
        ...
        val useCase = PageBuddyGroupMemoListTagFilterUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupMemoListTagFilterRepository = buddyGroupMemoListTagFilterRepository,
        )

        Given("로그인된 계정으로 그룹의 메모 목록 태그 필터를 페이징하는 경우") {
            val account = fixture.giveMeOne<Account.User>()
            val buddyGroupId = fixture.giveMeOne<Uuid>()
            val pagingData = PagingData.empty<TagFilter>()

            every { getAccountUseCase() } returns flowOf(Result.success(account))
            every { buddyGroupMemoListTagFilterRepository.page(buddyGroupId) } returns flowOf(pagingData)
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupMemoListTagFilterRepository.page(buddyGroupId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupMemoListTagFilterRepository.page(any())
                    }
                }
            }
        }
