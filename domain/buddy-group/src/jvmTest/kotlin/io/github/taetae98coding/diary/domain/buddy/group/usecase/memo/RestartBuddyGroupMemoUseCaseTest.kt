...
class RestartBuddyGroupMemoUseCaseTest : BehaviorSpec() {
    init {
        ...
        val useCase = RestartBuddyGroupMemoUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupMemoRepository = buddyGroupMemoRepository,
        )

        Given("로그인된 계정으로 메모를 재시작하는 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository를 호출한다") {
                    coVerify(exactly = 1) {
                        buddyGroupMemoRepository.updateFinished(
                            account = account,
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository를 호출하지 않는다") {
                    coVerify(exactly = 0) {
                        buddyGroupMemoRepository.updateFinished(any(), any(), any(), any())
                    }
                }
            }
        }
