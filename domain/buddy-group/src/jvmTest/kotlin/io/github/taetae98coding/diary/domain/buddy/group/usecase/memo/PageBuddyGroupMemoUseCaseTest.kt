...
class PageBuddyGroupMemoUseCaseTest : BehaviorSpec() {
    init {
        ...
        val buddyGroupMemoListRepository = mockk<BuddyGroupMemoListRepository>(relaxed = true, relaxUnitFun = true)
        val useCase = PageBuddyGroupMemoUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupMemoListRepository = buddyGroupMemoListRepository,
        )

        Given("로그인된 계정으로 그룹의 메모를 페이징하는 경우") {
            ...
            every { buddyGroupMemoListRepository.page(account, buddyGroupId) } returns flowOf(pagingData)
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupMemoListRepository.page(account, buddyGroupId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupMemoListRepository.page(any(), any())
                    }
                }
            }
        }
