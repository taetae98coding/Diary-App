...
class FetchBuddyGroupDiaryUseCaseTest : BehaviorSpec() {
    init {
        ...
        val buddyGroupDiaryRepository = mockk<BuddyGroupDiaryRepository>(relaxed = true, relaxUnitFun = true)

        val useCase = FetchBuddyGroupDiaryUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupDiaryRepository = buddyGroupDiaryRepository,
        )

        Given("로그인된 계정으로 그룹 일기를 불러오는 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository를 호출한다") {
                    coVerify(exactly = 1) {
                        buddyGroupDiaryRepository.fetch(
                            account = account,
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository를 호출하지 않는다") {
                    coVerify(exactly = 0) {
                        buddyGroupDiaryRepository.fetch(any(), any())
                    }
                }
            }
        }
