...
class PageBuddyGroupTagUseCaseTest : BehaviorSpec() {
    init {
        ...
        val useCase = PageBuddyGroupTagUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupTagRepository = buddyGroupTagRepository,
        )

        Given("로그인된 계정으로 그룹의 태그를 페이징하는 경우") {
            ...
            every { buddyGroupTagRepository.page(account, buddyGroupId) } returns flowOf(pagingData)
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupTagRepository.page(account, buddyGroupId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupTagRepository.page(any(), any())
                    }
                }
            }
        }
