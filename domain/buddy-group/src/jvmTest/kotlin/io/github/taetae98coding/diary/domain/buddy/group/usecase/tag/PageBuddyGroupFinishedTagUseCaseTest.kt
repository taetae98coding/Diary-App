...
class PageBuddyGroupFinishedTagUseCaseTest : BehaviorSpec() {
    init {
        ...
        val buddyGroupFinishedTagRepository = mockk<BuddyGroupFinishedTagRepository>(relaxed = true, relaxUnitFun = true)
        val useCase = PageBuddyGroupFinishedTagUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupFinishedTagRepository = buddyGroupFinishedTagRepository,
        )

        Given("로그인된 계정으로 그룹의 완료된 태그를 페이징하는 경우") {
            val account = fixture.giveMeOne<Account.User>()
            val buddyGroupId = fixture.giveMeOne<Uuid>()
            val pagingData = PagingData.empty<Tag>()

            every { getAccountUseCase() } returns flowOf(Result.success(account))
            every { buddyGroupFinishedTagRepository.page(account, buddyGroupId) } returns flowOf(pagingData)
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupFinishedTagRepository.page(account, buddyGroupId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupFinishedTagRepository.page(any(), any())
                    }
                }
            }
        }
