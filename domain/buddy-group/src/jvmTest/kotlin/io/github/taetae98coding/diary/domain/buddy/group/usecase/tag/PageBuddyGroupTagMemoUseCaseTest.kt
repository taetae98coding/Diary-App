...
class PageBuddyGroupTagMemoUseCaseTest : BehaviorSpec() {
    init {
        ...
        val buddyGroupTagMemoRepository = mockk<BuddyGroupTagMemoRepository>(relaxed = true, relaxUnitFun = true)
        val useCase = PageBuddyGroupTagMemoUseCase(
            getAccountUseCase = getAccountUseCase,
            buddyGroupTagMemoRepository = buddyGroupTagMemoRepository,
        )

        Given("로그인된 계정으로 그룹의 태그 메모를 페이징하는 경우") {
            ...
            every { buddyGroupTagMemoRepository.page(account, buddyGroupId, tagId) } returns flowOf(pagingData)

            When("유즈케이스를 구독하면") {
                val result = useCase(buddyGroupId, tagId).first()
                ...
                Then("Repository 호출한다") {
                    verify(exactly = 1) {
                        buddyGroupTagMemoRepository.page(account, buddyGroupId, tagId)
                    }
                }
                clearAllMocks()
            }
        ...
        Given("로그인하지 않은 경우") {
            ...
            When("유즈케이스를 구독하면") {
                ...
                Then("Repository 호출하지 않는다") {
                    verify(exactly = 0) {
                        buddyGroupTagMemoRepository.page(any(), any(), any())
                    }
                }
            }
        }
