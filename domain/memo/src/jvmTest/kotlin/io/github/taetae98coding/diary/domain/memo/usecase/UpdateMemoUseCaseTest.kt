...
class UpdateMemoUseCaseTest : BehaviorSpec() {
    init {
        ...
        Given("메모가 존재하고 제목이 빈 문자열인 경우") {
            ...
            val account = fixture.giveMeOne<Account>()
            val input = fixture.giveMeOne<MemoDetail>().copy(title = "")
            every { getAccountUseCase() } returns flowOf(Result.success(account))
            every { getMemoUseCase(memoId) } returns flowOf(Result.success(existed))

            When("유즈케이스를 실행하면") {
                val result = useCase(memoId, input)
                ...
                Then("빈 제목은 기존 제목으로 대체되어 업데이트된다") {
                    coVerify(exactly = 1) {
                        repository.update(account, memoId, withArg { it.title shouldBe existed.detail.title })
                    }
                }
            }
        }
        ...
        Given("메모가 존재하고 제목이 유효한 경우") {
            ...
            val existed = fixture.giveMeOne<Memo>()
            val account = fixture.giveMeOne<Account>()
            val input = fixture.giveMeOne<MemoDetail>()
            every { getAccountUseCase() } returns flowOf(Result.success(account))
            every { getMemoUseCase(memoId) } returns flowOf(Result.success(existed))
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("입력 제목 그대로 업데이트된다") {
                    coVerify(exactly = 1) {
                        repository.update(account, memoId, withArg { it.title shouldBe input.title })
                    }
                }
            }
        }
