...
class SelectMemoTagUseCaseTest : BehaviorSpec() {
    init {
        ...
        Given("로그인 사용자가 태그를 선택하는 경우") {
            ...
            When("유즈케이스를 실행하면") {
                ...
                Then("Repository와 백업 유즈케이스가 호출된다") {
                    coVerify(exactly = 1) { accountMemoTagRepository.updateMemoTag(account, memoId, tagId, true) }
