...
internal fun BuddyGroupAddScreen(
    ...
    addViewModel: BuddyGroupAddViewModel = koinViewModel(),
) {
    val state = rememberBuddyGroupAddScreenState()

    val floatingUiState by addViewModel.floatingUiState.collectAsStateWithLifecycle()
    val buddyPagingItems = addViewModel.buddyPagingData.collectAsLazyPagingItems()
    ...
    BuddyGroupAddScreen(
        ...
        onAdd = { addViewModel.add(state.detailState.detail) },
        onBuddyTitle = state::showBuddySearchDialog,
        modifier = modifier.onActionShortcut { addViewModel.add(state.detailState.detail) },
    )

    BuddySearchBottomSheetHost(
        state = state,
        stateHolder = addViewModel,
    )
    ...
    BuddyGroupAddScreenEffect(
        ...
        onEffect = addViewModel::clearEffect,
    )
}
...
private fun BuddyGroupAddScreenEffect(
    state: BuddyGroupAddScreenState,
    effectProvider: () -> BuddyGroupAddEffect,
    onEffect: () -> Unit,
) {
