...
public class CalendarFilterStateHolderTemplate(
    private val strategy: CalendarFilterStrategy,
    private val coroutineScope: CoroutineScope = MainImmediateScope(),
) : CalendarFilterStateHolder,
    AutoCloseable {
    override val pagingData: Flow<PagingData<TagFilter>> = strategy.pageTagFilter()
        .mapLatest { result ->
            result.fold(
                onSuccess = { it },
                onFailure = {
                    Logger.log("[CalendarFilterStateHolderTemplate] 태그 필터 페이지 로드 실패 : $it")
                    PagingData.loading()
                },
            )
