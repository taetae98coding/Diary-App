...
public fun CalendarScaffold(
    ...
    val colorTextEffect by colorTestStateHolder.effect.collectAsStateWithLifecycle()
    ...
    CalendarScaffoldColorTextEffect(
        state = state,
        effectProvider = { colorTextEffect },
        onEffect = colorTestStateHolder::clearEffect,
    )
}
...
private fun CalendarScaffoldColorTextEffect(
    state: CalendarScaffoldState,
    effectProvider: () -> CalendarColorTextEffect,
    onEffect: () -> Unit,
) {
    val coroutineScope = rememberCoroutineScope()
